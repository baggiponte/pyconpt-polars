...
@build:
  npx slidev build --base=/pyconpt-polars
...
[private]
  ...
  #! /usr/bin/env zsh
  if ! gh repo list baggiponte | grep --quiet "/\bpyconpt-polars\b"; then
    while true; do
      ...
      case $REPLY in
        [Yy])
          ...
          case $PRIVATE in
            [Yy])
              gh repo create baggiponte/pyconpt-polars --private --source=.
              ;;
            [Nn])
              gh repo create baggiponte/pyconpt-polars --public --source=.
              ;;
            *)
    ...
    while true; do
      ...
      case $REPLY in
        [Yy])
          git remote add origin git@github.com:baggiponte/pyconpt-polars
          break
          ;;
        [Nn])
          break
